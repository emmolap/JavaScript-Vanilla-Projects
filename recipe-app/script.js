...
async function getRandomMeal() {
  const resp = await fetch(
    "https://www.therealdb.com/api/json/v1/1/random.php"
  );
...
async function getMealById(id) {
  const resp = await fetch(
    "https://www.therealdb.com/api/json/v1/1/lookup.php?i=" + id
  );
...
async function getMealsBySearch(term) {
  const resp = await fetch(
    "https://www.therealdb.com/api/json/v1/1/search.php?s=" + term
  );
...
function removeMealLS(mealId) {
  ...
  localStorage.setItem(
    "mealsIds",
    JSON.stringify([mealsIds.filter((id) => id !== mealId), mealsid])
  );
}
